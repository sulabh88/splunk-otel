...
# Script to forcefully delete stuck pods in an OpenShift cluster
# Targets: Terminating pods, long-pending pods, port conflict pods, and Portworx pods

# Ensure oc is installed and authenticated
...
# Function to identify and delete stuck pods
delete_stuck_pods() {
    echo "---------------------------------------"
    ...
    echo "---------------------------------------"
    echo "Checking for pods with port conflicts or waiting states..."
    WAITING_PODS=$(oc get pods --all-namespaces -o json | jq -r '.items[] | select(.status.phase=="Pending" and .status.conditions[]?.reason=="ContainerCreating" and .status.conditions[]?.message | contains("port")) | [.metadata.namespace, .metadata.name] | join(" ")')
    
    if [ -n "$WAITING_PODS" ]; then
        echo "Found pods potentially stuck due to port conflicts:"
        echo "$WAITING_PODS"
        echo "Force deleting these pods..."
        while IFS= read -r line; do
            NAMESPACE=$(echo "$line" | awk '{print $1}')
        ...
        echo "No pods with port conflicts found."
    fi
